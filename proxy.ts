...
const fullstackRoutes = new Set([
  ...
  "/api/chat", "/api/discount-lead", "/api/admin/login", "/api/admin/logout", "/api/admin/overview", "/api/admin/chat-action",
...
export function proxy(request: NextRequest) {
  ...
  if (!fullstackRoutes.has(pathname)) return new NextResponse("Page not found", { status: 404 });
